...
onAuthStateChanged(auth, async (user) => {
  ...
  if (user) {
    ...
    if (userSnap.exists() && userSnap.data().config) {
      ...
      console.log('Configuração do usuário carregada com sucesso');
      setupLocaisAtendimento();
    } else {
      console.warn('Usuário autenticado mas sem configuração encontrada.');
      setupLocaisAtendimento();
    }
  } else {
...
function setupLocaisAtendimento() {
  console.log('setupLocaisAtendimento chamado', userConfig);
  ...
  if (userConfig && userConfig.locaisAtendimento) {
    ...
    console.warn('Nenhum local de atendimento encontrado no config.');
  }
