...
import { getFirestore, collection, getDocs, doc, getDoc, setDoc, updateDoc } from "https://www.gstatic.com/firebasejs/9.22.2/firebase-firestore.js";
...
async function carregarConfiguracoes(config) {
  if (config.locaisAtendimento?.length) {
    localSelect.innerHTML = `<option>${config.locaisAtendimento[0]}</option>`;
  }
  if (config.hospitais?.length) {
    hospitalSelect.innerHTML = `<option>${config.hospitais[0]}</option>`;
  }
